...
public class Quanlylophoc extends JPanel {
	...
	public Quanlylophoc() {
		...
		@Override// sự kiện nhập enteer sẽ nhảy xuống dòng khác 
				 ...
				 int key=e.getKeyCode();
				     if(e.getSource()==txtMaLop )
				     {
				        ...
				        { 
				        	...
				        	txtchuyennganh.requestFocusInWindow(); 
		...
		JLabel lblNewLabel_2 = new JLabel("CHUYÊN NGÀNH: ");
		lblNewLabel_2.setBounds(25, 130, 139, 29);
		lblNewLabel_2.setFont(new Font("Arial", Font.BOLD, 14));
		add(lblNewLabel_2);
		
		txtchuyennganh = new JTextField();
		txtchuyennganh .addKeyListener(new KeyAdapter() {
			@Override
			public void keyPressed(KeyEvent e) {
				 int key=e.getKeyCode();
				     if(e.getSource()==txtchuyennganh )
				     {
				        if(key==KeyEvent.VK_ENTER)
				        { 
		...
		txtchuyennganh.setBounds(159, 131, 213, 29);
		add(txtchuyennganh);
		...
		lblNewLabel_4.setBounds(20, 219, 116, 29);
		...
		btTimkiem.setIcon(new ImageIcon("src\\main\\java\\img\\Graphicloads-Colorful-Long-Shadow-Zoom.16.png"));
		...
		btLaydulieu.setBounds(465, 210, 133, 36);
		add(btLaydulieu);
		
		JButton btClear = new JButton("Clear ");
		btClear.setFont(new Font("Arial", Font.BOLD, 17));
		btClear.setBounds(629, 210, 136, 36);
		add(btClear);
		...
		lblNewLabel_4_1.setBounds(20, 272, 116, 29);
		add(lblNewLabel_4_1);
	...
	public static void setDataCombox()
	{
		cbMakhoa.removeAllItems();
		//List<String> listMaKhoa = connectLopHoc.getInstanse().Select();
		cbMakhoa.addItem("");
		for (String maKhoa : connectLopHoc.getInstanse().Select()) {
...
public void insertLopVaoCombobox(String tenlop){
	...
	public void Laydulieu() {
		...
		for(Model.Quanlylophoc kh : connectLopHoc.getInstanse().selectAll()) {
			insert(kh);
		}
	}
	public void updatett() {
		...
		{ int i = table.getSelectedRow();
		  ...
		  this.txtchuyennganh.setText(tenLop);
	...
	public void capnhatdulieu() {//cập nhật cho database
		...
		String Covanhoctap = txtCo_van_hoc_tap.getText();
		String maKhoa = (String) cbMakhoa.getSelectedItem();
		connectLopHoc.getInstanse().updateQuanlylophoc(maLop,chuyennganh, Covanhoctap, maKhoa);
		Laydulieu();
	}
	public void delete()
	{
		connectLopHoc.getInstanse().deleteQuanlylophoc(txtMaLop.getText());// truyền vào mã khóa và thực hiện hàm xóa bên lớp csdl 
		Laydulieu();
	...
	public void timkiem() {
		...
		for( Model.Quanlylophoc kh : connectLopHoc.getInstanse().Timkiem(this.txtTimkiem.getText())) {
			insert(kh);
		}
