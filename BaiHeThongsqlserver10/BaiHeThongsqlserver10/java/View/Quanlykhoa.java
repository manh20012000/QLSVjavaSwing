...
//		});
	...
	public void Themthongtin () {
		...
		if(khoaDAO.getInstanse().kiemtra(maKhoa)==false) {
			Khoa kh = new Khoa(maKhoa,tenKhoa);
			insert(kh);
			Quanlylophoc.cbMakhoa.addItem(maKhoa); 		 
			khoaDAO.getInstanse(). insertKhoa(maKhoa, tenKhoa);
			setNull();
		}
		else if(khoaDAO.getInstanse().kiemtra(maKhoa)==true) {
	...
	public void Laydulieu() {
		...
		for(Khoa kh : khoaDAO.getInstanse().selectAll()) {
			insert(kh);
		}
	}
	...
	public void capnhatdulieu() {//cập nhật cho database
		...
		String maKhoa = txtMakhoa.getText();
		String tenKhoa = txtTenkhoa.getText();
		khoaDAO.getInstanse().updateKhoa(maKhoa, tenKhoa);
		Laydulieu();
		setNull();
	}
	public void delete()
	{
		khoaDAO.getInstanse().deleteKhoa(txtMakhoa.getText());
		Laydulieu();
		setNull();
	}
	...
	public void searchKhoa()
	{
		...
		clearDataTable();
		Khoa khoa = khoaDAO.getInstanse().searchKhoa(txtMakhoa.getText());
		insert(khoa);
